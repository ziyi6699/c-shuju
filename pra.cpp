...
void change(listnode* head, int x, int p) {
    listnode* pp = head;
    for (int i = 0; i < x && pp != NULL; i++) {
        pp = pp->next;
    }
    if (pp != NULL) {
        pp->data = p;
        cout << "change successed" << endl;
    }
    else {
...
void insert(listnode* head, int x, int p) {
    listnode* pp = head;
    listnode* ppp = new listnode;
    for (int i = 0; i < x && pp != nullptr; i++) {
        pp = pp->next;
    }
    if (pp != nullptr) {
        ppp->data = p;
        ...
        cout << "can't find insert location" << endl;
    }
}

void de(listnode* head, int x) {
    listnode* pp = head, * ppp = head;
    for (int i = 0; i < x && pp != nullptr; i++) {
        ppp = pp;
        pp = pp->next;
    }
...
void des(listnode* head) {
    while (head != NULL) {
        listnode* pp = head;
        head = head->next;
        delete pp;
    }
    cout << "delete listnode finshed" << endl;
}
